...
def main(argv):
    if(len(argv) != 4):
        ...
        print("param 0: .")
        print("param 1: input *.FLS file")
        print("param 2: input FAL file")
        print("param 3: output FLS file")
        raise Exception("param cnt error!")

    # fls file
    try:
        f_fls = open(argv[1], "rb+")
    except IOError:
        print("not find %s file" % argv[1])
        raise
    else:
        magic_word = f_fls.read(4)
    ...
    try:
        ...
        # 第一步，产生一个 xx 大小的文件(sec + rtt)
        f_fls.seek(0)
        data = f_fls.read()
        f_out.write(data)

        # Write fal file to output file
        f_out.seek(52 * 1024)
        data = struct.pack('<B', 0xff) * (4 * 1024)
        f_out.write(data) 

        f_out.seek(52 * 1024)        
        f_out.write(f_fal.read(4 * 1024))
