...
/* Domain type could be modified in runtime, dafault value 0 is shanghai demain. */
int g_domain_type = 0;
...
/* Keep this function, as iotx_cmp_api.c may invoke it */
...
{
    if (0 == g_domain_type)
        return GUIDER_DIRECT_DOMAIN_CN;
    
    else if (1 == g_domain_type)
        return GUIDER_DIRECT_DOMAIN_SG;
    
    else if (2 == g_domain_type)
        return GUIDER_DIRECT_DOMAIN_JP;
    
    else if (3 == g_domain_type)
        return GUIDER_DIRECT_DOMAIN_US;
    
    else if (4 == g_domain_type)
        return GUIDER_DIRECT_DOMAIN_GER;
...
static void guider_get_url(char *buf, int len)
{
    ...
    else if (1 == g_domain_type){ /*southeast*/
    #if defined(ON_PRE)
